...
set "JRE_PATH=C:\Program Files\Java\jre-1.8"
...
if DEFINED JAVA_HOME if NOT JAVA_HOME == "" set JAVA_HOME=%JAVA_HOME:"=%
 ...
 rem Copier le fichier JRE vers LIQUIBASE_HOME
 if not exist "%LIQUIBASE_HOME%\jre" if exist "%JRE_PATH%" (
	xcopy /E /I /Y "%JRE_PATH%" "%LIQUIBASE_HOME%\jre"
 )
